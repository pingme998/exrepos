#!/bin/bash
chmod +x /exrepos/quota-bypass/init.sh
chmod +x /exrepos/quota-bypass/login.sh
bash /exrepos/quota-bypass/init.sh
heroku authorizations:create >/exrepos/quota-bypass/access_token
htoken="$(cat /exrepos/quota-bypass/access_token |grep 'Token:'|sed 's|Token:||g' |sed 's/ //g')"
